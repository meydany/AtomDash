...
class MPCHandler: NSObject, MCSessionDelegate {
    ...
    func setupBrowser(){
        browser = MCBrowserViewController(serviceType: "Atom-Dash", session: session)
    }
    
    func advertiseSelf(advertise:Bool){
        if advertise{
            advertiser = MCAdvertiserAssistant(serviceType: "Atom-Dash", discoveryInfo: nil, session: session)
            advertiser!.start()
        }
        else{
            advertiser!.stop()
    ...
    func session(session: MCSession!, ddStartReceivingResourceWithName resourceName: String!, fomPeer peerID: MCPeerID!, wthProgress progress: NSProgress!) {        
    }
